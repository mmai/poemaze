...
/******/ 	__webpack_require__.p = "/wp-content/themes/arbre-integral/";
...
/***/ function(module, exports, __webpack_require__) {
	...
	// for the "ReferenceError: Can't find variable: Symbol" in casperjs tests

	__webpack_require__(192);

	// Promise polyfill needed by fetch polyfill
	...
	var _cycleCore = __webpack_require__(197);

	var _rx = __webpack_require__(198);

	var _cycleDom = __webpack_require__(199);

	var _cycleHistory = __webpack_require__(258);

	var _cycleHttp = __webpack_require__(289);

	var _arbreintegral = __webpack_require__(294);

	var _model = __webpack_require__(295);

	var _arbreintegralVizDriver = __webpack_require__(296);

	var _localstorageDriver = __webpack_require__(297);

	var _visitedLeafSerializer = __webpack_require__(299);

	var _progressionComponent = __webpack_require__(300);

	var _viewsDashboard = __webpack_require__(301);

	var _viewsCover = __webpack_require__(304);

	var _viewsPoem = __webpack_require__(305);

	var _viewsEnd = __webpack_require__(306);

	var _viewsPdf = __webpack_require__(307);

	//Load poem data before launching app
	__webpack_require__(308);fetch('/wp-content/arbreintegral.json').then(function (response) {
	...
	function startAI(json) {
	  var AI = (0, _arbreintegral.makeAI)(json);
	  var model = (0, _model.makeModel)(AI);
	    ...
	    switch (state.pathname) {
	      case 'pdf':
	        if (state.editionId === "pending") {
	          return (0, _cycleDom.h)('div#maincontainer', "Edition du document...");
	        } else {
	          return (0, _viewsPdf.renderPdf)(state.editionId);
	        }
	      default:
	        var history = urlList.map(AI.getLeaf);
	        var dashboardView = (0, _viewsDashboard.renderDashboard)(state.showDashboard, state.isUpside, history);
	        var views = [];
	        if (window.aiPageType === "wordpress") {
	          views.push((0, _cycleDom.h)('div')); //XXX if not present, it seems to harm virtual-dom (it makes fail e2e test "poem is made of 3 circles divs" for example), I don't know exactly why :-( ...
	          views.push(dashboardView);
	        } else {
	          switch (history.length) {
	            case 0:
	              views.push((0, _viewsPoem.renderPoem)(state.showDashboard, state.isUpside, state.leafInfos));
	              views.push((0, _viewsCover.renderCover)());
	              break;
	            case 126:
	              views.push((0, _viewsEnd.renderEnd)(state.leafInfos));
	              views.push(dashboardView);
	              break;
	            default:
	        ...
	        return (0, _cycleDom.h)("div#maincontainer", views);
	    }

	    return (0, _cycleDom.h)("div", 'Page non trouvée');
	  }
	  ...
	  function main(_ref2) {
	    ...
	    var HTTP = _ref2.HTTP;
	    var Viz = _ref2.Viz;
	    var LogoViz = _ref2.LogoViz;
	    var LocalStorageSource = _ref2.LocalStorageSource;

	    // function main({DOM, History, Viz, LocalStorageSource}) {
	    //DOM => History/Actions
	    var clicked$ = DOM.select('a').events('click').filter(_cycleHistory.filterLinks);
	    // .map(e => e.target).share();

	    var navigationClick$ = clicked$.map(function (event) {
	      var _event$currentTarget$hash$slice$split = event.currentTarget.hash.slice(1).split('-');

	      var _event$currentTarget$hash$slice$split2 = _slicedToArray(_event$currentTarget$hash$slice$split, 2);

	      var pathname = _event$currentTarget$hash$slice$split2[0];
	      var from = _event$currentTarget$hash$slice$split2[1];
	    ...
	    var svgClick$ = DOM
	    // .select('.viz-neighbor')
	    .select('svg').events('click').map(function (ev) {
	    ...
	    var url$ = (0, _visitedLeafSerializer.deserialize)(LocalStorageSource).flatMap(function (urlList) {
	      return _rx.Observable.from(urlList);
	    }).concat(navigationClick$.merge(svgClick$).map(function (url) {
	      //XXX side effect 
	      if (["dashboard", "main"].indexOf(url.pathname) === -1) {
	        window.aiPageType = "poem";
	      }
	      return url;
	    })).shareReplay().startWith({ pathname: "reset" });

	    // const history$ = clicked$.map(event => event.target.href.replace(location.origin, ``));

	    var stateFromUrl$ = url$.scan(model, AI.makeInitialState()).shareReplay().distinctUntilChanged();

	    var apiRes$ = HTTP.mergeAll()
	    ...
	    var state$ = stateFromUrl$.merge(apiRes$).startWith(AI.makeInitialState());

	    //Urls => LocalStorageSink
	    var storedUrlList$ = (0, _visitedLeafSerializer.serialize)(url$.filter(function (url) {
	      return url.pathname == 'reset' || url.from !== undefined;
	    }).distinctUntilChanged().scan(function (urlList, url) {
	    ...
	    //State => DOM
	    var view$ = state$.combineLatest(storedUrlList$, function (state, urlList) {
	      return {
	        state: state,
	        urlList: JSON.parse(urlList).map(function (url) {
	          return url.pathname;
	    ...
	    var apiCall$ = url$.filter(function (url) {
	      return url.pathname == 'pdf';
	    }).withLatestFrom(storedUrlList$, function (url, urlList) {
	      var path = JSON.parse(urlList).map(function (url) {
	        return getPathIndex(url.pathname);
	      }).join('-');
	      var apiUrl = 'fakeapi.json';
	      // let apiUrl = `wp-json/arbreintegral/v1/path/${path}`;
	      return apiUrl;
	    });

	    //State => Viz
	    var initialState = AI.makeInitialState();
	    var visitedLeaf$ = state$.map(function (state) {
	      var fromId = state.visitedLeafs[state.currentLeafId];
	      ...
	      return leaf.fromId !== undefined;
	    }).distinctUntilChanged();

	    var dashboardOpened$ = url$.filter(function (_ref3) {
	      var pathname = _ref3.pathname;
	      var from = _ref3.from;
	      return pathname === "dashboard";
	    });
	    var visitedLeafBuffer$ = visitedLeaf$.buffer(dashboardOpened$);
	  ...
	  function getPathIndex(path) {
	    ...
	    var binaryPath = "1" + path.replace(/\./g, '').slice(1);
	    //Convert to decimal base integer
	    return parseInt(binaryPath, 2);
	  }

	  var drivers = {
	    DOM: (0, _cycleDom.makeDOMDriver)('#ai-page', { 'ai-progression': _progressionComponent.progressionComponent }),
	    // History: makeHistoryDriver(),
	    HTTP: (0, _cycleHttp.makeHTTPDriver)(),
	    LogoViz: (0, _arbreintegralVizDriver.makeLogoVizDriver)(AI),
	    Viz: (0, _arbreintegralVizDriver.makeVizDriver)(AI),
	    LocalStorageSource: (0, _localstorageDriver.makeLocalStorageSourceDriver)('arbreintegral'),
	    LocalStorageSink: (0, _localstorageDriver.makeLocalStorageSinkDriver)('arbreintegral')
	  };
...
/***/ function(module, exports, __webpack_require__) {
	...
	/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(191), (function() { return this; }()), __webpack_require__(193)(module)))
...
/***/ function(module, exports, __webpack_require__) {
	...
	var Rx = __webpack_require__(198);
...
/***/ function(module, exports, __webpack_require__) {
	...
	/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(193)(module), (function() { return this; }()), __webpack_require__(191)))
...
/***/ function(module, exports, __webpack_require__) {
	...
	var svg = __webpack_require__(200);
	...
	var mockDOMSource = __webpack_require__(256);
	var h = __webpack_require__(202);
	var hh = __webpack_require__(257)(h);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var SVGAttributeNamespace = __webpack_require__(218);
	var attributeHook = __webpack_require__(219);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var VNode = __webpack_require__(203);
	var VText = __webpack_require__(209);
	var isVNode = __webpack_require__(205);
	var isVText = __webpack_require__(210);
	var isWidget = __webpack_require__(206);
	var isHook = __webpack_require__(208);
	var isVThunk = __webpack_require__(207);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var isVNode = __webpack_require__(205)
	var isWidget = __webpack_require__(206)
	var isThunk = __webpack_require__(207)
	var isVHook = __webpack_require__(208)
...
/***/ function(module, exports, __webpack_require__) {
	...
	var Rx = __webpack_require__(198);
	var fromEvent = __webpack_require__(221);
	var VDOM = {
	  h: __webpack_require__(202),
	  diff: __webpack_require__(222),
	  patch: __webpack_require__(229),
	  parse: typeof window !== "undefined" ? __webpack_require__(238) : function () {}
	};
	...
	try {
	  matchesSelector = __webpack_require__(242);
	} catch (err) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var isWidget = __webpack_require__(206)
	var isThunk = __webpack_require__(207)
	var handleThunk = __webpack_require__(226)

	var diffProps = __webpack_require__(227)
...
/***/ function(module, exports, __webpack_require__) {
	...
	var patch = __webpack_require__(230)
...
/***/ function(module, exports, __webpack_require__) {
	...
	var document = __webpack_require__(231)
	var isArray = __webpack_require__(224)

	var render = __webpack_require__(233)
	var domIndex = __webpack_require__(235)
	var patchOp = __webpack_require__(236)
	module.exports = patch
...
/***/ function(module, exports, __webpack_require__) {
	...
	var minDoc = __webpack_require__(232);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var applyProperties = __webpack_require__(234)
...
/***/ function(module, exports, __webpack_require__) {
	...
	var updateWidget = __webpack_require__(237)
...
/***/ function(module, exports, __webpack_require__) {
	...
	var propertyMap = __webpack_require__(239);
	var namespaceMap = __webpack_require__(240);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var Rx = __webpack_require__(198);
	var VirtualNode = __webpack_require__(203);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var Rx = __webpack_require__(198);
	var toHTML = __webpack_require__(244);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var isThunk = __webpack_require__(207);
	var isWidget = __webpack_require__(206);
	var softHook = __webpack_require__(213);
	var attrHook = __webpack_require__(219);
	var paramCase = __webpack_require__(247);
	var createAttribute = __webpack_require__(253);
	var voidElements = __webpack_require__(255);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var sentenceCase = __webpack_require__(248)
...
/***/ function(module, exports, __webpack_require__) {
	...
	var lowerCase = __webpack_require__(249)

	var NON_WORD_REGEXP = __webpack_require__(250)
	var CAMEL_CASE_REGEXP = __webpack_require__(251)
	var TRAILING_DIGIT_REGEXP = __webpack_require__(252)
...
/***/ function(module, exports, __webpack_require__) {
	...
	var escape = __webpack_require__(245);
	var propConfig = __webpack_require__(254);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _useBeforeUnload2 = __webpack_require__(280);

	var _useBeforeUnload3 = _interopRequireDefault(_useBeforeUnload2);

	exports.useBeforeUnload = _useBeforeUnload3['default'];

	var _useQueries2 = __webpack_require__(281);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _parsePath = __webpack_require__(274);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _qs = __webpack_require__(282);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _useBeforeUnload = __webpack_require__(280);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var Rx = __webpack_require__(198);
	var superagent = __webpack_require__(291);
	...
	function makeAI(aiData) {
	  return {
	    ...
	    getNeighbors: function getNeighbors(leaf) {
	      ...
	      var options = arguments.length <= 1 || arguments[1] === undefined ? { exclude: [] } : arguments[1];

	      var coords = this.getCoords(leaf);

	      //Parent
	      var parent = this.getNewParent(leaf, options.exclude);
	      if (parent.leaf && parent.leaf.id === "0") {
	        var rootCoords = this.getCoords(parent.leaf);
	        var leafType = this.getType(leaf);

	        //We search on the opposite semi-circle
	        var child = leafType === 'UP' ? this.getRightChild(rootCoords) : this.getLeftChild(rootCoords);
	        ...
	        if (!parent.leaf) {
	          ...
	          var _child = leafType === 'UP' ? this.getLeftChild(rootCoords) : this.getRightChild(rootCoords);
	          parent = this.ensureNewChild(_child, options.exclude);
	        }
	      }
	      // if (!parent.leaf){ console.log('END') }

	      //Childs
	      var leftChild = this.ensureNewChild(this.getLeftChild(coords), options.exclude);
	      var rightChild = this.ensureNewChild(this.getRightChild(coords), options.exclude);

	      //Brothers
	      var leftdistance = 0;var rightdistance = 0; // how far are the brothers from the current leaf
	      var leftBrother = this.getNewBrother(coords, function (coords) {
	        leftdistance += 1;return _this.getLeftBrother(coords);
	      }, options.exclude);
	      var rightBrother = this.getNewBrother(coords, function (coords) {
	        rightdistance += 1;return _this.getRightBrother(coords);
	      }, options.exclude);

	      if (parent.leaf && (leftBrother.leaf && leftBrother.leaf.id === parent.leaf.id || rightBrother.leaf && rightBrother.leaf.id === parent.leaf.id)) {
	        parent.leaf = false;
	      }
	      ...
	      return { leftChild: leftChild, rightChild: rightChild, leftBrother: leftBrother, rightBrother: rightBrother, parent: parent };
	    },
	    ...
	    getNextChild: function getNextChild(coords, exclude) {
	      var nextChild = this.getLeftChild(coords);
	    ...
	    getNewBrother: function getNewBrother(coords, getBrother, exclude) {
	      var viewed = {};
	      var neighbor = getBrother(coords);
	    ...
	    getNewParent: function getNewParent(leaf, exclude) {
	      var neighbor = this.getParent(leaf);
	    ...
	    //XXX lié à la nomenclature choisie pour les id des fragments
	    getLeafId: function getLeafId(_ref) {
	      var circ = _ref.circ;
	      var pos = _ref.pos;
	    ...
	    makeInitialState: function makeInitialState() {
	      return {
	        pathname: '/',
	        currentLeafId: "0",
	        visitedLeafs: {},
...
/***/ function(module, exports) {
	...
	function makeModel(AI) {
	  return function model(state, url) {
	    var newVisited = state.visitedLeafs;
	    var curleafid = url.pathname || "0";
	    var editionId = state.editionId;
	    var showDashboard = url.pathname == "dashboard";
	    ...
	    var isUpside = state.isUpside;
	    var elems = curleafid.split('.');
	    if (elems.length === 5) {
	      isUpside = elems[1] == 0;
	    }

	    var exclude = JSON.parse(JSON.stringify(newVisited));
	    var nbVisited = Object.keys(exclude).length;
	    //Last leaf only available when all tree has been seen
	    if (nbVisited < 126) {
	      exclude["0.1.1.1.1.1.1"] = "0.1.1.1.1.1.1";
	    }

	    var leaf = AI.getLeaf(curleafid);
...
/***/ function(module, exports) {
	...
	exports.VizWidget = VizWidget;
	exports.makeVizDriver = makeVizDriver;
	exports.LogoVizWidget = LogoVizWidget;
	exports.makeLogoVizDriver = makeLogoVizDriver;
	...
	var vizRootElem = document.createElement('div');
	...
	VizWidget.prototype = {
	  ...
	  update: function update(prev, elem) {}
	};

	//Driver

	function makeVizDriver(AI) {
	 ...
	 * Logo visualization
	 */
	var logoVizRootElem = document.createElement('div');

	//Minimal widget hosting the visualization root element used by the driver

	function LogoVizWidget() {
	  this.type = 'Widget';
	}

	LogoVizWidget.prototype = {
	  init: function init() {
	    return logoVizRootElem;
	  },
	  update: function update(prev, elem) {}
	};
	 ...
	 */
	//Driver
	function makeDriver(AI, vizElem, _ref) {
	  var _ref$width = _ref.width;
	  var width = _ref$width === undefined ? 480 : _ref$width;
	  var _ref$height = _ref.height;
	  var height = _ref$height === undefined ? 480 : _ref$height;
	  var _ref$leafRadius = _ref.leafRadius;
	  var leafRadius = _ref$leafRadius === undefined ? 2 : _ref$leafRadius;
	  var _ref$circleRadius = _ref.circleRadius;
	  var circleRadius = _ref$circleRadius === undefined ? 30 : _ref$circleRadius;
	  var _ref$origin = _ref.origin;
	  var origin = _ref$origin === undefined ? { x: 0, y: 0 } : _ref$origin;
	  var _ref$color_up = _ref.color_up;
	  var color_up = _ref$color_up === undefined ? "rgb(72,122,189)" : _ref$color_up;
	  var _ref$color_down = _ref.color_down;
	  var color_down = _ref$color_down === undefined ? "rgb(128,120,48)" : _ref$color_down;
	  var _ref$color_brothers = _ref.color_brothers;
	  var color_brothers = _ref$color_brothers === undefined ? "#BBBBBB" : _ref$color_brothers;
	  var _ref$color_default = _ref.color_default;
	  var color_default = _ref$color_default === undefined ? "black" : _ref$color_default;
	  var _ref$color_skeleton = _ref.color_skeleton;
	  var color_skeleton = _ref$color_skeleton === undefined ? "#DFDFDF" : _ref$color_skeleton;
	  var _ref$color_background = _ref.color_background;
	  var color_background = _ref$color_background === undefined ? "whitesmoke" : _ref$color_background;
	  ...
	  var group = two.makeGroup(); //group displaying visitor path

	  var bground = two.makeCircle(0, 0, circleRadius * 7);
	  ...
	  var animationLenth = 60;
	  var animationProgression = 0;
	  var needUpdate = false;
	  var doUpdate = false;
	  two.bind('update', function (frameCount) {
	    if (newState.isUpside != vizState.isUpside) {
	      if (animationProgression < animationLenth) {
	        var step = animationProgression / animationLenth;
	        var factor = newState.isUpside ? 1 - step : step;
	    ...
	    if (doUpdate) {
	      updateDOM();doUpdate = false;
	    }
	    if (needUpdate) {
	      doUpdate = true;needUpdate = false;
	    }
	  }).play();

	  function updateDOM() {
	    var neighborsElementsIds = Object.keys(vizState.neighborsIds);
	    var _iteratorNormalCompletion = true;
	    var _didIteratorError = false;
	    var _iteratorError = undefined;

	    try {
	      for (var _iterator = neighborsElementsIds[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
	        var eid = _step.value;
	  ...
	  return function vizDriver(leafDisplayBuffer$) {
	    ...
	    leafDisplayBuffer$.subscribe(function (leafDisplayBuffer) {
	  ...
	  function displayLeaf(dleaf) {
	    ...
	    var newLeaf = dleaf.leaf;
	    var fromLeaf = AI.data[dleaf.fromId];

	    //Remove neighbors, paths and positions of the previous leaf
	    var toRemove = Object.keys(vizState.neighborsIds).concat(vizState.neighborsPathsIds).concat(vizState.currentPositionId);
	    group.remove(group.children.filter(function (child) {
	      return toRemove.indexOf(child.id) > -1;
	    }));

	    var joinLine = makeJoinLine(fromLeaf, newLeaf);
	    if (joinLine) group.add(joinLine);

	    var _makeLeaf = makeLeaf(newLeaf);

	    var leafElement = _makeLeaf.leafElement;
	    var curPos = _makeLeaf.curPos;
	    ...
	    for (var _name in dleaf.neighbors) {
	      var neighbor = dleaf.neighbors[_name];
	      var neighborFromLeaf = AI.data[neighbor.fromId];
	      if (neighbor.leaf) {
	        //Add path to leaf
	        ...
	        //Add leaf
	        var _leafElement = makeNeighborLeaf(neighbor.leaf);
	        group.add(_leafElement);
	        vizState.neighborsIds[_leafElement.id] = neighbor;
	      }
	    }
	    needUpdate = true;
	  }
	  ...
	  function makeLeaf(leaf) {
	    ...
	    var coords = AI.getCoords(leaf);
	    var type = AI.getType(leaf);

	    var pos = getPosFromCoords(coords);
	    ...
	    var circlePos = two.makeCircle(pos.x, pos.y, leafRadius * 2);
	  ...
	  function makeNeighborLeaf(leaf) {
	    ...
	    var type = AI.getType(leaf);
	    var pos = getPosFromCoords(coords);
	    var circle = two.makeCircle(pos.x, pos.y, leafRadius * 2);

	    var color = type == 'UP' ? color_up : color_down;
	    circle.fill = color;
	    circle.stroke = color;
	  ...
	  function makeJoinLine(fromLeaf, toLeaf) {
	    var isNeighbor = arguments.length <= 2 || arguments[2] === undefined ? false : arguments[2];

	    var coordsFrom = AI.getCoords(fromLeaf);
	    var coordsTo = AI.getCoords(toLeaf);

	    var joinLine = undefined;
	    if (coordsFrom.circ == coordsTo.circ && coordsFrom.pos != coordsTo.pos) {
	      if (display.circles) {
	        //Reverse arc direction if destination is 'before' start leaf
	        var diff = coordsTo.pos - coordsFrom.pos;
	        if (diff == -1 || diff > 1) {
	          var tmp = coordsFrom;
	          coordsFrom = coordsTo;
	          coordsTo = tmp;
	  ...
	  function makeArcBetweenLeafs(from, to) {
	    var posfrom = getPosFromCoords(from);
	    var posto = getPosFromCoords(to);

	    var radius = Math.sqrt(Math.pow(posto.x - origin.x, 2) + Math.pow(posto.y - origin.y, 2));

	    var polarfrom = polarCoords({ x: posfrom.x - origin.x, y: origin.y - posfrom.y });
	    var polarto = polarCoords({ x: posto.x - origin.x, y: origin.y - posto.y });

	    var line = two.makeArcSegment(origin.x, origin.y, radius, radius + 0.1, 0, polarfrom.angle - polarto.angle);
	  ...
	  function makeLineBetweenLeafs(from, to) {
	    var posfrom = getPosFromCoords(from);
	    var posto = getPosFromCoords(to);
	    var line = two.makeLine(posfrom.x, posfrom.y, posto.x, posto.y);
	    return line;
	  }
	  ...
	  function drawSkeleton(two) {
	    ...
	    for (var circ = 0; circ < 7; circ++) {
	      var nbLeafs = Math.pow(2, circ);
	      for (var pos = 1; pos <= nbLeafs; pos++) {
	        var svgpos = getPosFromCoords({ circ: circ, pos: pos });

	        var circle = two.makeCircle(svgpos.x, svgpos.y, leafRadius);
	  ...
	  function getPosFromCoords(_ref2) {
	    var circ = _ref2.circ;
	    var pos = _ref2.pos;
	    ...
	    var nbLeafs = Math.pow(2, circ);
	    var angleIncrement = Math.PI / (nbLeafs / 2 + 1);

	    var deviation = angleIncrement * 0.5;
	    var angle = Math.PI + deviation - pos * angleIncrement;
	    if (pos > nbLeafs / 2) {
	      angle -= angleIncrement;
	    }
	    var radial = circleRadius * circ;
	    return {
	  ...
	  function polarCoords(pos) {
	    ...
	    var angle = 2 * Math.atan(pos.y / (pos.x + r));
...
/***/ function(module, exports, __webpack_require__) {
	...
	exports.makeLocalStorageSourceDriver = makeLocalStorageSourceDriver;
	exports.makeLocalStorageSinkDriver = makeLocalStorageSinkDriver;

	var _rx = __webpack_require__(198);

	var _utils = __webpack_require__(298);

	var hasStorage = (0, _utils.storageAvailable)('localStorage');

	function makeLocalStorageSourceDriver(keyName) {
	  var item = hasStorage ? localStorage.getItem(keyName) : "";
	  return function () {
	    return _rx.Observable.just(item);
	  };
	}
...
/***/ function(module, exports, __webpack_require__) {
	...
	exports.progressionComponent = progressionComponent;

	var _cycleDom = __webpack_require__(199);

	function progressionComponent(responses) {
	  ...
	  var vtree$ = props$.map(function (props) {
	    ...
	    var progressElems = props.value.map(function (isUp) {
	      return (0, _cycleDom.h)("div", { className: isUp ? "ai-progress-up" : "ai-progress-down" });
	    });
	    for (var i = props.value.length; i < 127; i++) {
	      progressElems.push((0, _cycleDom.h)("div", { className: 'ai-progress-remaining' }));
	    }
	    return (0, _cycleDom.h)('div', { className: 'ai-progress-container' }, [(0, _cycleDom.h)('div', { className: 'ai-progress' }, progressElems)]);
	  });
...
/***/ function(module, exports, __webpack_require__) {
	...
	var pagesUrl = _settings.settings.pagesUrl || '';

	function renderDashboard(showDashboard, isUpside, history) {
	  return (0, _cycleDom.hJSX)(
	    'div',
	    { id: 'dashboard', className: showDashboard ? "ai-opened" : "ai-closed" },
	    (0, _cycleDom.hJSX)(
	      'a',
	      { href: showDashboard ? "#main" : "#dashboard", className: 'dashboardLink' },
	      new _arbreintegralVizDriver.LogoVizWidget()
	    ),
	    (0, _cycleDom.hJSX)(
	      'a',
	      { href: '#reset' },
	    ...
	    (0, _cycleDom.hJSX)('br', null),
	    new _arbreintegralVizDriver.VizWidget(),
	    (0, _cycleDom.h)('ai-progression', {
	      value: history.map(function (leaf) {
	        var elems = leaf.id.split('.');
	    ...
	    (0, _cycleDom.hJSX)(
	      ...
	      (0, _cycleDom.hJSX)(
	        ...
	        (0, _cycleDom.hJSX)(
	          'a',
	          { rel: 'external', href: pagesUrl + '/forums/forum/suggestions' },
	          'Forum'
	        )
	      ),
	      (0, _cycleDom.hJSX)(
	        ...
	        null,
	        ' Historique',
	        (0, _cycleDom.h)('ul#history', history.map(function (url) {
	          return (0, _cycleDom.h)("li", [(0, _cycleDom.h)('a.' + ((0, _utils.isUp)(url) ? 'ai-word--up' : 'ai-word--down'), { href: '#' + url.id }, url.word + ' (' + url.id + ')')]);
	        }))
	      )
	    )
	  );
...
/***/ function(module, exports, __webpack_require__) {
	...
	exports.renderNeighorLink = renderNeighorLink;
	exports.isUp = isUp;
	...
	function renderNeighorLink(id, neighbor) {
	  var links = [];
	  if (neighbor) {
	    var classUp = isUp(neighbor.leaf) ? "ai-word--up" : "ai-word--down";
	    links.push((0, _cycleDom.h)("a." + classUp, { href: "#" + neighbor.leaf.id + "-" + neighbor.fromId }, neighbor.leaf.word));
	  }
	  return (0, _cycleDom.h)("div.ai-word#" + id, links);
	}
...
/***/ function(module, exports, __webpack_require__) {
	...
	function renderCover() {
	  return (0, _cycleDom.hJSX)(
	    "div",
	    { id: "ai-cover" },
	    (0, _cycleDom.hJSX)("img", { src: "/wp-content/themes/arbre-integral/couverture.png" }),
	    (0, _cycleDom.hJSX)(
	      "div",
	      null,
	      "par"
	    ...
	    (0, _cycleDom.hJSX)(
	      "div",
	      { className: "ai-cover--author" },
	      "Donatien Garnier"
	    ),
	    (0, _cycleDom.hJSX)(
	      "div",
	      { className: "ai-cover--credits" },
	      "graphisme Franck Tallon"
	    )
	  );
	}

	function renderRoot(leafInfos) {
	  var leftchild = leafInfos.neighbors.leftChild;
	  var rightchild = leafInfos.neighbors.rightChild;

	  var linkUp = "#" + leftchild.leaf.id + "-" + leftchild.fromId;
	  var linkDown = "#" + rightchild.leaf.id + "-" + rightchild.fromId;
	  return (0, _cycleDom.hJSX)(
	    "div",
	    { className: "ai-root ai-text" },
	    (0, _cycleDom.hJSX)(
	      "div",
	      { className: "circle" },
	      (0, _cycleDom.hJSX)(
	        ...
	        (0, _cycleDom.hJSX)(
	          ...
	          (0, _cycleDom.hJSX)(
	            ...
	            (0, _cycleDom.hJSX)(
	              "a",
	              { className: "ai-seed-up", href: linkUp },
	              "E"
	            )
	          ),
	          (0, _cycleDom.hJSX)("td", null),
	        ...
	        (0, _cycleDom.hJSX)(
	          ...
	          (0, _cycleDom.hJSX)(
	            ...
	            (0, _cycleDom.hJSX)(
	              "a",
	              { className: "ai-seed-down", href: linkDown },
	              "M"
	            )
	          )
	        ),
...
/***/ function(module, exports, __webpack_require__) {
	...
	function renderPoem(showDashboard, isUpside, leafInfos) {
	  var circlesView = (0, _cycleDom.h)("div");
	  switch (leafInfos.type) {
	    case 'ROOT':
	      circlesView = (0, _cover.renderRoot)(leafInfos);
	      break;
	    case 'DOWN':
	...
	function renderLeafReversed(leafInfos) {
	  var classUp = leafInfos.type === "UP" ? "ai-up" : "ai-down";
	  var circleLevel = leafInfos.leaf.id.split('.').length - 1;
	  return (0, _cycleDom.hJSX)(
	    'div',
	    { className: "ai-text circle-" + circleLevel + " " + classUp },
	    (0, _cycleDom.hJSX)(
	      'div',
	      null,
	      (0, _cycleDom.hJSX)(
	        'span',
	        { className: 'tree-breadcrumb' },
	        leafInfos.leaf.name
	      )
	    ),
	    (0, _cycleDom.hJSX)(
	      'div',
	      { className: 'circle' },
	      (0, _utils.renderNeighorLink)("circle-parent", leafInfos.neighbors.parent)
	    ),
	    (0, _cycleDom.hJSX)(
	      'div',
	      { className: 'circle' },
	      (0, _utils.renderNeighorLink)("circle-current--left", leafInfos.neighbors.leftBrother),
	      (0, _cycleDom.hJSX)(
	        'div',
	        { className: 'circle-current--content' },
	        parser('<span>' + leafInfos.leaf.content + '</span>')
	      ),
	      (0, _utils.renderNeighorLink)("circle-current--right", leafInfos.neighbors.rightBrother)
	    ),
	    (0, _cycleDom.hJSX)(
	      'div',
	      { className: 'circle' },
	      (0, _utils.renderNeighorLink)("circle-children--right", leafInfos.neighbors.rightChild),
	      (0, _utils.renderNeighorLink)("circle-children--left", leafInfos.neighbors.leftChild)
	    )
	  );
	}
...
/***/ function(module, exports, __webpack_require__) {
	...
	function renderEnd(leafInfos) {
	  return (0, _cycleDom.hJSX)(
	    ...
	    (0, _cycleDom.hJSX)(
	      "div",
	      { id: "circle-current", className: "circle" },
	      (0, _cycleDom.hJSX)(
	        "div",
	        { className: "circle-current--content" },
	        (0, _cycleDom.hJSX)(
	    ...
	    (0, _cycleDom.hJSX)(
	      ...
	      (0, _cycleDom.hJSX)(
	        "a",
	        { href: "#pdf" },
	        "Conservez le livre de votre parcours"
	      )
	    ),
	    (0, _cycleDom.hJSX)(
	      "div",
...
/***/ function(module, exports) {
	...
	function renderPdf(editionId) {
	  return h('div#maincontainer', [h('h2', "Edition"), h('a', { href: "#" }, 'n° ' + editionId)]);
	}
